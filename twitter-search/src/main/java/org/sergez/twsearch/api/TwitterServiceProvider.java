...
public class TwitterServiceProvider {
	...
	public void onLoadTweets(final SearchTweetsTagEvent event) {
     mApi.getTweetTagList("Bearer " + event.twitterToken, event.id, new Callback<List<TrendList>>() {
			@Override
			public void success(List<TrendList> response, Response rawResponse) {
		...
		mApi.getTweetList("Bearer " + event.twitterToken, event.id, new Callback<TweetList>() {
			@Override
